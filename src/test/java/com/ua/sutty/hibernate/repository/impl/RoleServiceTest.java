...
public class RoleServiceTest {
    ...
    public void testFindByName() throws Exception {
        assertEquals("Role after findByName must coincide with role in data base",
                roleService.findRoleByName("USER").getName(), iDatabaseTester.getDataSet().getTable("role").getValue(0, "name"));
    }

    @Test
    public void testFindById() throws Exception {
        assertEquals("Role after findById must coincide with role in data base",
                roleService.findRoleById(1L).getName(), iDatabaseTester.getDataSet().getTable("role").getValue(0, "name"));
    }
    ...
    public void testSave() throws Exception {
        ...
        assertEquals("Size database must be increase by one after save",
                4, iDatabaseTester.getConnection()
                        .createDataSet().getTable("role").getRowCount());
        assertEquals("Role must be in base after save", role.getName()
                , roleService.findRoleByName("MARKETER").getName());
    }
    ...
    public void testUpdate() throws Exception {
        ...
        roleService.save(role);
        assertEquals("After update role name must be change",
                roleService.findRoleById(role.getId()), role);
